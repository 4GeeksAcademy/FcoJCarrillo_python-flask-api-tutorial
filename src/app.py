...
def handle_users():
    ...
    if request.method == 'GET':
        response_body['message'] ='Estos son los datos de todos estos usuarios'
        response_body['result'] = users
        return response_body, 200
    if request.method == 'POST':
        data = request.json
        users.append(data)
        response_body['message'] = ''
        #Solamente aparecerá el último que añadimos
        response_body['result'] = users[-1]
        return response_body, 200
...
def handle_user(id_user):
    ...
    if id_user >= len(users):
        response_body['message'] = f'No existe el usuario con id {id_user}'
        response_body['result'] = {}
    if request.method == 'GET':
        response_body['message'] = f'Datos de la tarea {id_user}'
        response_body['results'] = users[id_user]
        return response_body, 200
        pass
    if request.method == 'PUT':
        users[id_user] = request.json
        response_body['message'] = f'Tarea {id_user} modificada'
        response_body['results'] = todos[id_user]
        return response_body, 200
        pass
